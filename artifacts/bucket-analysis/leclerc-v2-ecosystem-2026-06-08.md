...
- ✓ Native worklet required env, adapter-present status mapping, missing-adapter
  log/error copy, component-level not-configured errors, station scaffold public
  key, and injectable QVAC/WDK/P2P handler router use the shared worklet story
  contract:
  `packages/core/src/worklet-stories.ts`, `packages/worklet/src/index.ts`,
  `apps/desktop/src/main.ts`, `apps/desktop/src/bridge.ts`, and
  `apps/mobile/src/worklet-client.ts`.
...
- ✓ Mobile declares Expo, React Native, `react-native-bare-kit`, and
  `bare-pack`; registers an Expo root component; renders the shared mobile model;
  and packs the placeholder Bare worklet for iOS/Android:
  `apps/mobile/package.json`, `apps/mobile/app.json`,
  `apps/mobile/index.ts`, `apps/mobile/src/App.native.ts`,
  `apps/mobile/src/worklet/index.js`, and
  `artifacts/native/mobile-bare-bundle-proof-2026-07-29.md`.
- ◐ Desktop does not yet vendor Electron/Pear or render a window.
- ◐ Mobile now declares Expo/React Native/Bare and bundles a placeholder
  worklet, but it still does not render an installable app artifact.
- ◐ Native worklet hosts now report adapter-present `not-configured` for
  QVAC/WDK/Hyperswarm; real SDK handlers are still not on-device in
  desktop/mobile.
...
3. Implement the native worklet SDK handlers for QVAC, WDK, and Hyperswarm,
   verifying installed `.d.ts` and QVAC examples before each call.
