...
| B11 | Repro, artifacts, compliance gates | 99% | Updated status notes now track shared field-demo fixture extraction, structured-error smoke coverage, story-owned descriptors, notification-store bridge verification, browser-alert verification, Link/voice/settings/SPY i18n key coverage, RAG/dossier/intel/station/QVAC runtime/model-level/inference-mode/vault/network-token/wallet-tool/tool-router/transfer/API-error/API-client/diagnostic/P2P/PWA-notification story prompt/report filename/feed-limit verification, PWA brand metadata, native brand identity, assistant persona/RAG chip display, blank-source detection, and finance story verification. |
